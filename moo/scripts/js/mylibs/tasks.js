...
(function() {
  var TASK_MAPPING, Task, TasksViewModel, hack, prVarValues, prZVarValues,
  ...
  hack = function(mappingOptions) {
    var actualVariables, hackTaskVars, hackVariablesMap, taskName, variable, workflowName, _i, _len, _ref, _results;
    workflowName = mappingOptions.parent.processInstanceId().split(".")[0];
    if (workflowName === "Personnel_Recovery") {
      hackVariablesMap = prVarValues;
    } else if (workflowName === "A2C2-PR-Z") {
      hackVariablesMap = prZVarValues;
    }
    if (hackVariablesMap == null) {
      return;
    }
    ...
    if (hackTaskVars == null) {
      console.log("Null hack vars");
    }
    _results = [];
    for (_i = 0, _len = hackTaskVars.length; _i < _len; _i++) {
      variable = hackTaskVars[_i];
